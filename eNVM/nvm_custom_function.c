		...
		uint32_t num_of_pages,  uint8_t expected_byte)
{
	...
	for (uint32_t current_page = 0; current_page < num_of_pages; current_page++)
	{
		...
		errors_in_page = find_bit_flips_in_page(this_uart, page_address, expected_byte);

		errors_found += errors_in_page;
	}

	sprintf (msg, "\n\r[BITFLIP]: Found errors: %d", errors_found);
	MSS_UART_polled_tx_string (this_uart, msg);
