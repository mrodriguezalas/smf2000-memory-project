...
//#define 	SDRAM_END		0xA0800001u //800000 bytes
#define 	SDRAM_END		0xA07FFFFFu
...
//Function that write 0 to all cells that are in the range adrStart to adrStop
{
	uint8_t msg[120];
	// Print to UART
	sprintf (msg, "\n\r\tInitializing cells %#X to %#X to 0 values", adrStart, adrStop);
...
int main (void)
{
	...
	//{
		...
		gen_ran_adr = random_address(&g_mss_uart0, 14, SDRAM_START, SDRAM_END);
		fault_injector_static(&g_mss_uart0, gen_ran_adr, 1);
		/// test static fault
		MSS_UART_polled_tx_string (&g_mss_uart0,"\n\r\tRead_0");
		read_and_compare_to_zeros_SDRAM(&g_mss_uart0, SDRAM_START, SDRAM_END);
		*/
		while(1){};
	//}
	return 0;
